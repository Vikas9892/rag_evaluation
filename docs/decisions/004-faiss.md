...
> **2026-08 — the Lambda constraint no longer applies.** The deployment target
> changed to EC2 (ADR 007 is superseded), so the 250 MB package limit cited below
> is no longer a binding constraint. **The decision stands on its remaining
> grounds** — CPU-only inference, resident memory on a 2 GiB instance, and MTEB
> score per MB — and the shipped model is unchanged. Kept as written rather than
> edited, because the reasoning is what the record is for.
